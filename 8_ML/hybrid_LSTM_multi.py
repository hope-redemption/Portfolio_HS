...
class hybridRNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        self.lstm_bi = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, bidirectional=True)
        self.lstm = nn.LSTM(hidden_size * num_layers, hidden_size, num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        # print(x)
        out, _ = self.lstm_bi(x, (h0, c0))  # out: tensor of shape (batch_size, seq_length, hidden_size*2)
        # print(out.shape)
        # print(out)
        out = self.dropout(out)

        h1 = torch.zeros(self.num_layers, out.size(0), self.hidden_size).to(device)
        c1 = torch.zeros(self.num_layers, out.size(0), self.hidden_size).to(device)

        out, _ = self.lstm(out, (h1, c1))  # out: tensor of shape (batch_size, seq_length, hidden_size)
        out = self.dropout(out)
        # print(out.shape)
        # print(out)
...
model = hybridRNN(input_size, hidden_size, num_layers, num_classes).to(device)
